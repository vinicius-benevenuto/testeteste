...
const SIP = (function(){
  ...
  function renderResult(data, rowIdx){
    ...
    // Montar valor único: "PL: x.x.x.x/28 | JG: x.x.x.x/28"
    const pl    = data.pl_bloco || '—';
    const jg    = data.jg_bloco || '—';
    const valor = `PL:${pl} / JG:${jg}`;

    // Auto-preencher o campo da linha
    fillBlocoIP(valor, rowIdx);

    // Exibir card de confirmação
    metaEl.textContent = `CN ${data.cn} · RN1 ${data.rn1_consultado}`;
    resultEl.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem .75rem;font-size:.78rem;margin-top:.25rem">
        <div>
          <span style="font-size:.68rem;color:var(--sub);display:block;margin-bottom:.15rem">Descrição</span>
          <span style="font-weight:600">${data.descricao||'—'}</span>
        </div>
        <div>
          <span style="font-size:.68rem;color:var(--sub);display:block;margin-bottom:.15rem">VLAN</span>
          <span style="font-weight:600">${data.vlan||'—'}</span>
        </div>
        <div>
          <span style="font-size:.68rem;color:var(--sub);display:block;margin-bottom:.15rem">CDSIP_SPO_PL</span>
          <span style="font-family:monospace;font-weight:600;color:var(--p)">${pl}</span>
        </div>
        <div>
          <span style="font-size:.68rem;color:var(--sub);display:block;margin-bottom:.15rem">CDSIP_SPO_JG</span>
          <span style="font-family:monospace;font-weight:600;color:var(--p)">${jg}</span>
        </div>
      </div>
      <div style="margin-top:.5rem;font-size:.7rem;color:var(--sub)">${data.mensagem||''}</div>`;
