...
class PTIWorkbookBuilder:
    ...
    def _b_routing(self):
        ...
        # Larguras de coluna fixas (mesmas do modelo original)
        self._cw(ws, {
            1: 2, 2: 15, 3: 10, 4: 15, 5: 10, 6: 18,
            7: 15, 8: 18, 9: 12, 10: 8, 11: 8, 12: 8,
            13: 8, 14: 8, 15: 8, 16: 12, 17: 18, 18: 12,
            19: 12, 20: 15, 21: 12, 22: 10, 23: 12, 24: 10, 25: 2,
        })
        ...
        # ===== FONTE DE DADOS (mesmo padrão do Diagrama) =====
        source_rows = self.vivo_rows if self.vivo_rows else self.op_rows
        num_blocks = max(1, len(source_rows))
        is_vivo_source = bool(self.vivo_rows)
        traffic_str = " / ".join(t for t in self.traffic if t) or ""

        # Linhas de dados adicionais (vazias) abaixo da preenchida
        EXTRA_EMPTY_ROWS = 4

        cursor = 6  # linha inicial do primeiro bloco
        ...
        for b_idx in range(num_blocks):
            ...
            mask_op = mask_vivo  # geralmente compartilhada
            ...
            # ----- SEPARADOR DO BLOCO: CN / Cidade / UF -----
            r = cursor
            ...
            for sc, ec, t, vertical in header_groups_l1:
                if vertical:
            ...
            # Sub-header nível 2: IP VIVO / IP Operadora
            ...
            # =============================================================
            # LINHA DE DADOS PREENCHIDA (1ª linha = dados da row atual)
            # =============================================================
            ds = h3 + 1
            data_map = {
                2:  area_local,           # ÁREA LOCAL
                3:  cn_vivo,              # CN (VIVO)
                4:  localidade_vivo,      # POI/PPI VIVO
                5:  cn_op,                # CN (Operadora)
                6:  localidade_op,        # POI/PPI Operadora
                7:  sbc_vivo,             # PONTA A VIVO
                8:  sbc_op,               # PONTA B Operadora
                9:  traffic_str,          # TIPO DE TRÁFEGO
                10: "",                   # BANDA EXIST.
                11: "",                   # BANDA PLAN.
                12: "",                   # CANAIS EXIST.
                13: "",                   # CANAIS PLAN.
                ...
                19: "",                   # CODEC
                20: "",                   # OBSERVAÇÃO
                21: endereco_vivo,        # IP ADDRESS VIVO
                22: mask_vivo,            # NETMASK VIVO
                23: ip_op,               # IP ADDRESS OPERADORA
                24: mask_op,             # NETMASK OPERADORA
            }

            # Renderizar a linha preenchida
            ws.row_dimensions[ds].height = 22.0
            f0 = s.alt_fill(0)
            for col in range(2, 25):
                c = ws.cell(row=ds, column=col, value=data_map.get(col, ""))
                c.font = s.font_small
                c.alignment = s.align_center
                c.border = s.box_border
                c.fill = f0

            # ----- LINHAS ADICIONAIS VAZIAS (para preenchimento manual) -----
            for i in range(1, EXTRA_EMPTY_ROWS + 1):
                r = ds + i
                ws.row_dimensions[r].height = 22.0
                f = s.alt_fill(i)
                for col in range(2, 25):
            ...
            # ----- AVISO DE DADOS AUSENTES (se algum campo-chave faltou) -----
            missing = []
            if not cn_vivo:
                missing.append("CN VIVO")
            if not sbc_vivo:
                missing.append("SBC VIVO")
            if not area_local:
                missing.append("ÁREA LOCAL")
            if not endereco_vivo:
                missing.append("ENDEREÇO IP VIVO")
            if missing:
                note_row = ds + EXTRA_EMPTY_ROWS + 1
                ws.merge_cells(start_row=note_row, start_column=2,
                               end_row=note_row, end_column=24)
                note_text = f"⚠ Campos não preenchidos neste bloco: {', '.join(missing)}"
