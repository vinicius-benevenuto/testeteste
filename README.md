...
<script>
/* Injeta CSS no documento pai (Streamlit) IMEDIATAMENTE ao carregar o iframe.
   Como é same-origin, window.parent.document é acessível.
   Isso ocorre antes de qualquer rerender do Streamlit, eliminando o flash
   do "Drag and drop file here" no primeiro carregamento. */
(function injectParentCSS() {{
  try {{
    var pd = window.parent.document;
    if (pd.getElementById('__vivohub_hide_dropzone')) return;
    var s = pd.createElement('style');
    s.id = '__vivohub_hide_dropzone';
    s.textContent = [
      ...
      '{{ display: none !important; }}',
      '[data-testid="stComponentContainer"] section',
      '{{ display: none !important; }}',
...
function exportCSV() {{
  ...
  downloadBlob(blob, 'tabela_filtrada_' + ts + '.csv');
}}
...
function exportExcel() {{
  ...
  const xmlRows = [cols, ...rows.map(r => cols.map(k => String(r[k] ?? '')))];
  const wsRows  = xmlRows.map((row, ri) =>
    '<Row ss:Index="' + (ri+1) + '">' +
    row.map(v =>
  ...
  const ts   = new Date().toISOString().slice(0,16).replace('T','_').replace(':','');
  downloadBlob(blob, 'tabela_filtrada_' + ts + '.xls');
}}
