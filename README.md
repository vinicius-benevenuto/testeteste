...
# =============================================================================
# SBC — PROCESSADOR DE DADOS CSV
# =============================================================================
class SBCDataProcessor:
    """
    Lê, limpa e cacheia dados de SBC a partir de CSVs.
    
    Responsabilidades:
    - Encontrar o CSV mais recente no diretório configurado
    - Ler o CSV (encoding UTF-8, separador vírgula)
    ...
    def find_latest_csv(self) -> Optional[str]:
        """
        Encontra o CSV mais recente no diretório por data de modificação.
        Busca por extensões .csv e .CSV.
        Retorna None se nenhum CSV for encontrado.
        """
        if not os.path.isdir(self.csv_dir):
            logger.warning(f"Diretório de SBC CSVs não encontrado: {self.csv_dir}")
            return None
        ...
        if not csv_files:
            logger.warning(f"Nenhum CSV encontrado em: {self.csv_dir}")
            return None

        # Ordena por data de modificação (mais recente primeiro)
        csv_files.sort(key=lambda f: os.path.getmtime(f), reverse=True)
        latest = csv_files[0]
        logger.info(f"CSV mais recente: {os.path.basename(latest)} "
                     f"({len(csv_files)} arquivos no diretório)")
        return latest
    ...
    def _read_csv(self, filepath: str) -> tuple[list[SBCMeasurement], list[dict]]:
        """
        Lê o CSV e retorna TODAS as linhas organizadas:
        ...
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                # Mapeia colunas do CSV para nomes internos
                col_mapping = {}
                ...
                for row_num, row in enumerate(reader, start=2):
                    ...
                    if sbc_name and uf:
                        # Linha com SBC individual identificado
                        ...
                        # Linha agregada (sem SBC/UF) — PRESERVADA, nunca descartada
                        aggregated.append({
                            "row_num": row_num,
    ...
    def get_data(self, force_reload: bool = False) -> tuple[list[SBCMeasurement], list[dict]]:
        ...
        if force_reload or not self._cache_data or cache_expired:
            ...
            if (latest_csv == self._cache_file_path
            ...
            # Carrega do disco
            logger.info(f"Carregando CSV do disco: {os.path.basename(latest_csv)}")
            measurements, aggregated = self._read_csv(latest_csv)

            # Atualiza cache
            self._cache_data = measurements
            self._cache_aggregated = aggregated
            self._cache_timestamp = now
            self._cache_file_path = latest_csv
...
class SBCAnalyzer:
    ...
    def health_check(self) -> dict:
        ...
        csv_info = self.processor.get_csv_info()
        csv_path = self.processor.find_latest_csv()
