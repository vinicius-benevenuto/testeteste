...
<body class="{{ 'plain' if plain_bg else '' }}">
  ...
  <nav class="v-nav">
    {% set _auth_pages = ['auth.login', 'auth.admin_login'] %}
    {% set _logged_in = session.get('user_id') or session.get('is_admin') %}
    {% if _logged_in and request.endpoint not in _auth_pages %}
    {% set _role = session.get('role') %}
    {% if _role == 'atacado' %}
      <a class="btn-g btn-sm" href="{{ url_for('central.central_atacado') }}">← Voltar</a>
    {% elif _role == 'engenharia' %}
      <a class="btn-g btn-sm" href="{{ url_for('central.central_engenharia') }}">← Voltar</a>
    {% else %}
      <a class="btn-g btn-sm" href="{{ url_for('auth.logout') }}" onclick="return false;" style="visibility:hidden">← Voltar</a>
    {% endif %}
    {% endif %}
