...
def _resolve_diagram_image() -> str:
    """Resolve o caminho da imagem do diagrama.
    
    Ordem de busca:
    1. Variável de ambiente DIAGRAM_IMAGE_PATH
    2. Caminhos relativos ao projeto (static/ e templates/)
    3. Diretórios vizinhos (VIVOHUB/, VIVOHUB_backup/, etc.)
    4. Busca recursiva por qualquer PNG nos diretórios próximos
    """
    ...
    # 3. Pasta irmã VIVOHUB (caso o projeto esteja em vivo_hub/ e a imagem em VIVOHUB/)
    ...
    # 4. Busca recursiva por PNG nos diretórios próximos (máx 2 níveis)
    for search_base in [base, parent]:
        try:
            for root, dirs, files in _os.walk(search_base):
                # Não entrar em pastas muito profundas ou irrelevantes
                depth = root.replace(search_base, "").count(_os.sep)
                if depth > 2:
                    dirs.clear()
                    continue
    ...
    # Retornar env var ou fallback mesmo se não encontrado
    # (builder vai logar erro informativo)
    if env:
        return env
