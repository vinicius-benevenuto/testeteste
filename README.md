...
class PTIWorkbookBuilder:
    ...
    def reserva_redes_ipam(self, vivo_rows: list[dict]) -> None:
        ...
        for item in vivo_rows:
            ...
            # mask agora está em Dados Operadora — buscar pela linha com mesmo CN
            op_match   = next((o for o in self.op_rows if o.get("cn","") == cn_usuario), None)
            mask_alvo  = (op_match.get("mask","") if op_match else "") or item.get("mask","")
