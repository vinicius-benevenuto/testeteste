...
<style>
  ...
  /* ── SBC Panel ── */
  .sbc-panel {
    margin-top:.75rem; border:1px solid #e2d5f0; border-radius:10px;
    background:linear-gradient(135deg,#faf8ff,#fff); padding:1rem; display:none;
  }
  .sbc-panel-head { display:flex; justify-content:space-between; align-items:center; margin-bottom:.65rem; }
  .sbc-panel-title { font-size:.82rem; font-weight:700; color:var(--p); }
  .sbc-panel-meta  { font-size:.7rem; color:var(--sub); }
  .sbc-list { max-height:400px; overflow-y:auto; }
  .sbc-card {
    border:1px solid var(--bdr); border-radius:8px; background:#fff;
    padding:.6rem .75rem; margin-bottom:.4rem;
    transition:border-color .15s, box-shadow .15s;
  }
  .sbc-card:hover { border-color:#c4a9e0; box-shadow:0 2px 8px rgba(107,9,166,.07); }
  .sbc-card.rec   { border-color:var(--ok); border-width:2px; }
  .sbc-card.crit  { border-color:#dc2626; border-style:dashed; background:#fff5f5; }
  .sbc-head { display:flex; justify-content:space-between; align-items:center; gap:.5rem; margin-bottom:.35rem; }
  .sbc-name { font-size:.82rem; font-weight:700; }
  .sbc-loc  { font-size:.72rem; color:var(--sub); margin-left:.3rem; }
  .sbc-badge {
    font-size:.65rem; font-weight:700; padding:.12rem .45rem;
    border-radius:999px;
  }
  .sbc-disp { background:#d1fae5; color:#065f46; }
  .sbc-mod  { background:#fef3c7; color:#92400e; }
  .sbc-crit { background:#fee2e2; color:#991b1b; }
  .sbc-bar-wrap { height:4px; background:#f3f4f6; border-radius:2px; overflow:hidden; margin:.3rem 0; }
  .sbc-bar { height:100%; border-radius:2px; transition:width .3s; }
  .bar-ok  { background:var(--ok); }
  .bar-mid { background:#f59e0b; }
  .bar-bad { background:#dc2626; }
  .sbc-data { display:grid; grid-template-columns:1fr 1fr; gap:.15rem .75rem; font-size:.72rem; color:#555; }
  .sbc-foot { display:flex; align-items:center; gap:.5rem; margin-top:.4rem; }
  .sbc-score { font-size:.78rem; font-weight:700; }
  .sbc-reason { font-size:.68rem; color:var(--sub); flex:1; line-height:1.3; }
  .sbc-use { font-size:.72rem; font-weight:600; padding:.22rem .55rem; border-radius:6px; white-space:nowrap;
             border:1px solid var(--bdr); background:#fff; cursor:pointer; }
  .sbc-use:hover { border-color:var(--p); color:var(--p); background:var(--p-lt); }
  .sbc-crit .sbc-use { border-color:#fecaca; color:#dc2626; }
  .sbc-crit .sbc-use:hover { background:#fee2e2; }
  .sbc-divider { font-size:.7rem; font-weight:700; display:flex; align-items:center; gap:.4rem; margin:.5rem 0 .3rem; }
  .sbc-divider::before,.sbc-divider::after { content:''; flex:1; height:1px; }
  .sbc-ok-d { color:var(--ok); }
  .sbc-ok-d::before,.sbc-ok-d::after { background:#a7f3d0; }
  .sbc-bad-d { color:#dc2626; }
  .sbc-bad-d::before,.sbc-bad-d::after { background:#fecaca; }
  .sbc-msg { font-size:.78rem; color:var(--sub); text-align:center; padding:.6rem; }
  .sbc-fallback { font-size:.72rem; color:#92400e; background:#fffbeb; border:1px solid #fde68a; border-radius:6px; padding:.3rem .6rem; margin-bottom:.4rem; display:none; }
  .sbc-summary { display:flex; gap:.5rem; flex-wrap:wrap; align-items:center; margin-bottom:.5rem; font-size:.72rem; }
  .sbc-chip { padding:.15rem .4rem; border-radius:999px; font-weight:600; }
  ...
  >
    ...
    <div class="sec">
      ...
      <div class="sec-body" style="padding:.75rem">
        <div class="data-sec">
          <table id="tableVivo">
            <thead><tr>
              ...
              <th>CN</th><th>SBC</th><th>Mask</th><th>Endereço Link</th>
        ...
        <!-- Painel SBC -->
        <div class="sbc-panel" id="sbcSuggestionPanel">
          <div class="sbc-panel-head">
            <span class="sbc-panel-title" id="sbcPanelTitle">SBCs</span>
            <span class="sbc-panel-meta" id="sbcPanelMeta"></span>
          </div>
          <div class="sbc-fallback" id="sbcFallbackNote"></div>
          <div class="sbc-list" id="sbcCardList"></div>
          <div class="sbc-msg" id="sbcLoading" style="display:none">Buscando…</div>
          <div class="sbc-msg" id="sbcEmpty"   style="display:none"></div>
        </div>
      </div>
    </div>
    {% endif %}
...
function serVivo(){
  return Array.from(document.querySelectorAll('#tableVivo tbody tr')).map(tr=>{
    ...
    return {ref:c[0]?.querySelector('input')?.value||'',data:c[1]?.querySelector('input')?.value||'',escopo:c[2]?.querySelector('input')?.value||'',localidade:c[3]?.querySelector('input')?.value||'',cn:c[4]?.querySelector('input')?.value||'',sbc:c[5]?.querySelector('input')?.value||'',mask:c[6]?.querySelector('input')?.value||'',endereco_link:c[7]?.querySelector('input')?.value||'',cidade:c[8]?.querySelector('input')?.value||'',uf:c[9]?.querySelector('input')?.value||'',lat:c[10]?.querySelector('input')?.value||'',long:c[11]?.querySelector('input')?.value||''};
  });
}
function serOp(){
...
const SBC=(function(){
  ...
  if(!panel||!ENG) return {fetch:()=>{},fill:()=>{}};
  let _ri=-1,_last='',_tmr=null;
  const num=(v,fb)=>{ const n=Number(v); return isNaN(n)?fb:n; };
  const fmt1=v=>{ const n=num(v,0); return Number.isFinite(n)?n.toFixed(1):n; };
  const scoreClass=s=>s>=70?'bar-ok':s>=40?'bar-mid':'bar-bad';
  const bClass=s=>({disponivel:'sbc-disp',moderado:'sbc-mod',critico:'sbc-crit'})[s]||'sbc-mod';
  const bLabel=s=>({disponivel:'Disponível',moderado:'Moderado',critico:'Crítico'}[s]||s||'—');
  const srcBadge=st=>{
    if(!st) return '';
    const k=st.toLowerCase();
    let c='background:#f3f4f6;color:#374151';
    if(k.includes('norm')) c='background:#d1fae5;color:#065f46';
    else if(k.includes('aten')) c='background:#fef3c7;color:#92400e';
    else if(k.includes('crit')||k.includes('crít')) c='background:#fee2e2;color:#991b1b';
    return `<span style="font-size:.65rem;font-weight:700;padding:.1rem .4rem;border-radius:4px;${c}">${st}</span>`;
  };
  const tendHTML=t=>{
    if(!t||t==='estavel') return '<span style="font-size:.7rem;color:var(--sub)">→ estável</span>';
    if(t==='subindo')  return '<span style="font-size:.7rem;color:#dc2626">↗ subindo</span>';
    return '<span style="font-size:.7rem;color:var(--ok)">↘ descendo</span>';
  };

  function renderCards(data){
    listEl.innerHTML=''; loadEl.style.display='none'; emptyEl.style.display='none';
    titleEl.textContent=`SBCs — ${data.uf||'??'}${data.cidade?' ('+data.cidade+')':''} · CN ${data.cn||'—'}`;
    metaEl.innerHTML=data.source_file?`<i class="bi bi-file-earmark-text"></i> ${data.source_file}${data.source_modificado_em?' · '+data.source_modificado_em:''}`:'';
    if(data.fallback_usado){ fallEl.style.display='block'; fallEl.innerHTML=`Sem SBCs para ${data.uf}. Exibindo dados de: <strong>${data.fallback_origem}</strong>.`; } else { fallEl.style.display='none'; }
    if(!data.sbcs||!data.sbcs.length){ emptyEl.style.display='block'; emptyEl.textContent=data.mensagem||'Nenhum SBC encontrado.'; panel.style.display='block'; return; }
    const disp=data.sbcs.filter(s=>s.saude!=='critico'), crit=data.sbcs.filter(s=>s.saude==='critico');
    // Summary
    const sb=document.createElement('div'); sb.className='sbc-summary';
    sb.innerHTML=`<span style="font-weight:600">${data.sbcs.length} SBC(s)</span>${disp.length?`<span class="sbc-chip" style="background:#d1fae5;color:#065f46">✔ ${disp.length} disponível(is)</span>`:''} ${crit.length?`<span class="sbc-chip" style="background:#fee2e2;color:#991b1b">⚠ ${crit.length} crítico(s)</span>`:''}`;
    listEl.appendChild(sb);
    if(disp.length){ const d=document.createElement('div'); d.className='sbc-divider sbc-ok-d'; d.textContent=`Disponíveis (${disp.length})`; listEl.appendChild(d); disp.forEach(s=>listEl.appendChild(buildCard(s,false))); }
    if(crit.length){ const d=document.createElement('div'); d.className='sbc-divider sbc-bad-d'; d.textContent=`Críticos (${crit.length})`; listEl.appendChild(d); crit.forEach(s=>listEl.appendChild(buildCard(s,true))); }
    panel.style.display='block';
  }

  function buildCard(sbc,isCrit){
    const card=document.createElement('div');
    card.className='sbc-card'+( sbc.recomendado&&!isCrit?' rec':'')+( isCrit?' crit':'');
    const score=num(sbc.score,0);
    let meta='';
    if(sbc.cidade) meta+=`<span><i class="bi bi-geo-alt" style="font-size:.65rem"></i> ${sbc.cidade}/${sbc.uf||''}</span> `;
    if(sbc.responsavel) meta+=`<span><i class="bi bi-person" style="font-size:.65rem"></i> ${sbc.responsavel}</span> `;
    if(sbc.dia_mais_recente) meta+=`<span><i class="bi bi-clock" style="font-size:.65rem"></i> ${sbc.dia_mais_recente}</span>`;
    card.innerHTML=`
      <div class="sbc-head">
        <span class="sbc-name">${sbc.nome||'—'}<span class="sbc-loc">${sbc.cidade||''}</span></span>
        <div style="display:flex;gap:.3rem;align-items:center">
          ${srcBadge(sbc.status_fonte||'')}
          <span class="sbc-badge ${bClass(sbc.saude)}">${bLabel(sbc.saude)}</span>
          ${sbc.recomendado&&!isCrit?'<span class="sbc-badge" style="background:var(--p);color:#fff">✓ Rec.</span>':''}
          ${isCrit?'<span class="sbc-badge sbc-crit">⚠ Crítico</span>':''}
        </div>
      </div>
      <div class="sbc-bar-wrap"><div class="sbc-bar ${scoreClass(score)}" style="width:${Math.min(100,Math.max(2,score))}%"></div></div>
      <div class="sbc-data">
        <span>Modelo: <b>${sbc.modelo||'—'}</b></span>
        <span>Serviços: ${Array.isArray(sbc.servicos)?sbc.servicos.join(', '):(sbc.servicos||'—')}</span>
        <span>CAPS avg: <b>${fmt1(sbc.caps_avg)}</b> · máx ${num(sbc.caps_max,0)} · mín ${num(sbc.caps_min,0)}</span>
        <span>Último: <b>${num(sbc.caps_ultimo,0)}</b> · ${tendHTML(sbc.caps_tendencia)}</span>
      </div>
      ${meta?`<div style="display:flex;gap:.5rem;flex-wrap:wrap;font-size:.68rem;color:var(--sub);margin-top:.3rem">${meta}</div>`:''}
      <div class="sbc-foot">
        <span class="sbc-score" style="color:${score>=70?'var(--ok)':score>=40?'#f59e0b':'#dc2626'}">${score}/100</span>
        <span class="sbc-reason">${(sbc.motivo||'').split(' | ').slice(0,2).join(' · ')}</span>
        <button type="button" class="sbc-use" data-sbc="${sbc.nome||''}" data-crit="${isCrit?'1':'0'}">
          Usar na linha ${_ri+1}
        </button>
      </div>`;
    card.querySelector('.sbc-use').addEventListener('click',function(){
      const n=this.dataset.sbc, crit=this.dataset.crit==='1';
      if(crit&&!confirm(`⚠ SBC "${n}" está CRÍTICO.\nDeseja usá-lo mesmo assim?`)) return;
      fill(n,_ri);
    });
    return card;
  }

  function fill(sbcName,rowIdx){
  ...
  async function fetchSBC(cn,ri,uf){
    ...
    const qk=cn.length>=2?`cn=${cn}`:`uf=${uf}`;
    const ck=qk+':'+ri;
    if(ck===_last&&_ri===ri) return;
    _last=ck; _ri=ri;
    clearTimeout(_tmr);
    _tmr=setTimeout(async()=>{
      ...
      try{
        const r=await fetch(`/api/sbc/suggest?${qk}`);
        if(!r.ok) throw new Error(r.status);
        renderCards(await r.json());
      }catch(e){
        loadEl.style.display='none'; emptyEl.style.display='block';
        emptyEl.textContent='Erro ao buscar SBCs.';
      }
    },320);
  }
  return {fetch:fetchSBC,fill};
})();
...
function wireCN(tr){
  ...
  const cnH=()=>{
    ...
    setDirty();
    if(ENG&&v.length===2){ clearTimeout(t1); t1=setTimeout(()=>{ const ri=Array.from(tr.parentElement.children).indexOf(tr); SBC.fetch(v,ri,m.uf||''); },150); }
  };
  const ufH=()=>{
    const v=(ufI?.value||'').toUpperCase().replace(/[^A-Z]/g,'').slice(0,2); if(ufI) ufI.value=v; setDirty();
    if(ENG&&v.length===2){ clearTimeout(t2); t2=setTimeout(()=>{ const cv=(cnI.value||'').replace(/\D/g,''); const ri=Array.from(tr.parentElement.children).indexOf(tr); SBC.fetch(cv.length>=2?cv:'',ri,v); },150); }
  };
  ['input','change','blur'].forEach(ev=>{ cnI.addEventListener(ev,cnH); });
  if(ufI) ['input','change','blur'].forEach(ev=>{ ufI.addEventListener(ev,ufH); });
  if((cnI.value||'').trim().length>=2) cnH();
  else if((ufI?.value||'').trim().length>=2) ufH();
}
