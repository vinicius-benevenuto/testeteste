...
def _render_excel_filters(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, col in enumerate(row_cols):
            with grid[i]:
                ...
                selected = st.multiselect(
                    ...
                    key=f"flt_{col}", placeholder="Todos os valores",
    ...
    with sc2:
        if n_active and st.button("Limpar filtros", key="btn_clr_flt",
            ...
            st.session_state["excel_filters"] = {c: [] for c in BUSINESS_COLS}
            # Invalida exports em cache — precisam ser regerados com novos filtros
            for _k in ("export_xlsx_data", "export_pptx_geral_data"):
                st.session_state[_k] = None
            for _k in list(st.session_state.keys()):
                if _k.startswith("export_pptx_col_"):
                    st.session_state[_k] = None
            st.rerun()

    # Invalida exports em cache se os filtros mudaram desde a última geração
    _flt_hash = str(sorted((k, tuple(v)) for k, v in filters.items() if v))
    if st.session_state.get("_last_flt_hash") != _flt_hash:
        st.session_state["_last_flt_hash"] = _flt_hash
        for _k in ("export_xlsx_data", "export_pptx_geral_data"):
            st.session_state[_k] = None
        for _k in list(st.session_state.keys()):
            if _k.startswith("export_pptx_col_"):
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        'Use o botao de filtro nas colunas para filtrar. '
        'Para baixar os dados filtrados use os botoes "Baixar CSV" e "Baixar Excel" '
        'que aparecem dentro da tabela, abaixo da paginacao.</p>',
        unsafe_allow_html=True,
    )
