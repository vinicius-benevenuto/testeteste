...
class PTIWorkbookBuilder:
    ...
    def _b_prefixes(self):
        ...
        TITLE_W = 4     # col 2 width already set to 4

        # Indicadores: (label, tipo, valor_do_form)
        # tipo: "text" = mostra valor ou ✘ / "binary" = ✔ ou ✘
        indicators = [
            ("RN1",            "text",   self.rn1),
            ...
            ("CNG:",           "binary", self.cng),
            ("RN2",            "binary", False),  # sem campo no formulário
        ]
        ...
        for i, (label, tipo, valor) in enumerate(indicators):
            ...
            # Resposta (col 3-4 mescladas) — indicador visual
            ...
            if tipo == "binary":
                if valor:
                    cr_cell.value = "✔"
                    cr_cell.font = s.font_check  # verde
                else:
                    cr_cell.value = "✘"
                    cr_cell.font = s.font_x  # vermelho
            else:  # text
                if valor and str(valor).strip():
                    cr_cell.value = str(valor).strip()
                    cr_cell.font = s.font_small
                else:
                    cr_cell.value = "✘"
                    cr_cell.font = s.font_x  # vermelho

            # Bordas restantes (5-10)
            for col in range(5, 11):
                cx = ws.cell(row=r, column=col, value="")
                cx.border = s.box_border
                cx.fill = f
