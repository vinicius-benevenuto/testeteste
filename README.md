    Quero que você atue como um modelo de linguagem altamente especializado em leitura e interpretação de código. Vamos trabalhar a partir desta versão que enviarei a seguir:
...
def render_interactive_table(
    ...
    data_json = json.dumps(safe.to_dict(orient="records"), ensure_ascii=False)
...
function filtered(){{
  ...
  if(SCOL){{r=[...r].sort((a,b)=>{{const av=String(a[SCOL]||''),bv=String(b[SCOL]||'');return SDIR==='asc'?av.localeCompare(bv,'pt'):bv.localeCompare(av,'pt');}}); }}
  return r;
}}
...
function rHead(){{
  ...
  COLS.forEach(col=>{{
    ...
    th.querySelector('.thn').onclick=()=>{{
      if(col.key===SCOL)SDIR=SDIR==='asc'?'desc':'asc';else{{SCOL=col.key;SDIR='asc';}}
      SC=col.key;window.parent.postMessage({{type:'col_click',col:col.key}},'*');PAGE=0;render();
    }};
...
function rPag(total){{
  ...
  const mk=(l,cb,d,on)=>{{const el=document.createElement('button');el.className='pb'+(on?' on':'');el.textContent=l;el.disabled=d;el.onclick=cb;b.appendChild(el);}};
...
render();
    ...
    Instruções:
    - Leia atentamente.
    - Interprete de forma profunda.
    - Entenda cada detalhe estrutural, lógico e semântico.
    - Não tome nenhuma ação além de ler e compreender.
    - Não execute, não avalie, não otimize, não sugira nada.
    - Não me retorne análises, resumos, explicações ou qualquer forma de opinião.
    - Apenas leia, interprete e esteja pronto.
    - Aguarde até que eu dê o próximo comando.
    - Responda apenas com: “Lido. Pronto para o próximo passo.”
