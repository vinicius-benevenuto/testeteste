...
# ─── Wrappers cacheados — evitam re-geração a cada render ─────────────────────
# st.cache_data usa hash do DataFrame: só recalcula quando os dados mudam.

@st.cache_data(show_spinner=False)
def _cached_excel(df: pd.DataFrame) -> bytes:
    return _make_excel_professional(df)


@st.cache_data(show_spinner=False)
def _cached_pptx_general(df: pd.DataFrame) -> bytes:
    return build_general_pptx(df)


@st.cache_data(show_spinner=False)
def _cached_pptx_col(df: pd.DataFrame, col: str) -> bytes:
    return build_column_pptx(df, col)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  4. EXPORTAÇÕES — SEMPRE O ÚLTIMO ELEMENTO
    #     Excel e PPTX são gerados com @st.cache_data para não bloquear
    #     o render da página em cada interação.
    # ══════════════════════════════════════════════════════════════════════
    _divider()
    ...
    # Excel — cacheado por hash do DataFrame
    with ex2:
        try:
            xlsx_bytes = _cached_excel(df_exp)
            st.download_button(
    ...
    with ex3:
        try:
            pptx_gen = _cached_pptx_general(df_exp)
            st.download_button(
    ...
    # PPTX Coluna — cacheado
    with ex4:
        if sel_col:
            try:
                pptx_col = _cached_pptx_col(df_exp, sel_col)
                st.download_button(
