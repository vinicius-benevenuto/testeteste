...
function wireCN(tr){
  ...
  const cnH=()=>{
    ...
    if(v.length===2){
      ...
      SIP.query(v, ri);  // SIP.query tem debounce interno de 320ms
    }
  };
...
(function(){
  ...
  // Consultar siprouter para todas as linhas com CN preenchido ao carregar
  // Delay 700ms > 200ms (wireCN) + 320ms (SIP interno) = 520ms → garante que wireCN já rodou
  setTimeout(()=>{
    const rn1Val=(document.getElementById('rn1')?.value||'').trim();
    if(!rn1Val) return;
    SIP.resetCache();  // forçar nova consulta independente do cache do wireCN
    document.querySelectorAll('#tableVivo tbody tr').forEach((tr,ri)=>{
      const cnI=tr.querySelector('[data-field="cn"]');
      const cn=(cnI?.value||'').replace(/\D/g,'');
      if(cn.length===2) SIP.query(cn, ri);
