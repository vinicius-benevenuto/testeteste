...
class PTIWorkbookBuilder:
    ...
    def _b_routing(self):
        ...
        # Número de linhas de dados por tabela (tráfego define o mínimo)
        DATA_ROWS = max(5, len(traffic_items))
        ...
        for b_idx in range(num_blocks):
            ...
            #
            # CN ÚNICO POR COLUNA:
            #   - Colunas 3 (CN VIVO) e 5 (CN OP) fazem merge vertical
            #     cobrindo todas as DATA_ROWS linhas.
            #   - O CN aparece 1 vez, centralizado verticalmente.
            #
            # ÁREA LOCAL (col 2): também merge vertical (1 instância).
            #
            # Linha 1 = dados preenchidos; demais = estrutura vazia.
            # =============================================================
            ds = h3 + 1  # primeira linha de dados

            # --- MERGE VERTICAL: ÁREA LOCAL (col 2) ---
            ws.merge_cells(start_row=ds, start_column=2,
                           end_row=ds + DATA_ROWS - 1, end_column=2)
            c = ws.cell(row=ds, column=2, value=area_local)
            c.font = Font(name="Calibri", size=10, bold=True)
            c.alignment = Alignment(horizontal="center", vertical="center",
                                    wrap_text=True)
            c.fill = s.alt_fill(0)
            c.border = s.box_border
            _track_width(2, area_local)

            # --- MERGE VERTICAL: CN VIVO (col 3) ---
            ws.merge_cells(start_row=ds, start_column=3,
                           end_row=ds + DATA_ROWS - 1, end_column=3)
            c = ws.cell(row=ds, column=3, value=cn_vivo)
            c.font = Font(name="Calibri", size=10, bold=True)
            c.alignment = Alignment(horizontal="center", vertical="center")
            c.fill = s.alt_fill(0)
            c.border = s.box_border

            # --- MERGE VERTICAL: CN OPERADORA (col 5) ---
            ws.merge_cells(start_row=ds, start_column=5,
                           end_row=ds + DATA_ROWS - 1, end_column=5)
            c = ws.cell(row=ds, column=5, value=cn_op)
            c.font = Font(name="Calibri", size=10, bold=True)
            c.alignment = Alignment(horizontal="center", vertical="center")
            c.fill = s.alt_fill(0)
            c.border = s.box_border

            # --- Dados por-linha (tráfego + endereços) ---
            for i in range(DATA_ROWS):
                ...
                # Preencher todas as colunas (exceto as já merged: 2, 3, 5)
                row_data = {}

                # Col 4: POI/PPI VIVO (só 1ª linha)
                row_data[4] = localidade_vivo if i == 0 else ""
                # Col 6: POI/PPI Operadora (só 1ª linha)
                row_data[6] = localidade_op if i == 0 else ""
                # Col 7: PONTA A VIVO = SBC VIVO (só 1ª linha)
                row_data[7] = sbc_vivo if i == 0 else ""
                # Col 8: PONTA B OP = SBC OP (só 1ª linha)
                row_data[8] = sbc_op if i == 0 else ""
                # Col 9: TIPO DE TRÁFEGO (1 por linha de tráfego)
                row_data[9] = traf_val
                # Col 10-15: BANDA/CANAIS/CAPS (vazios)
                for empty_col in range(10, 16):
                    row_data[empty_col] = ""
                # Col 17: Encaminhamento A>B (só 1ª linha)
                row_data[17] = ""
                # Col 18: Encaminhamento B>A
                row_data[18] = ""
                # Col 19: CODEC
                row_data[19] = ""
                # Col 20: OBSERVAÇÃO (nota SBC na 1ª linha se auto-resolvido)
                if i == 0 and sbc_vivo_note:
                    row_data[20] = sbc_vivo_note
                else:
                    row_data[20] = ""
                # Col 21: IP ADDRESS VIVO (só 1ª linha)
                row_data[21] = endereco_vivo if i == 0 else ""
                # Col 22: NETMASK VIVO (só 1ª linha)
                row_data[22] = mask_vivo if i == 0 else ""
                # Col 23: IP ADDRESS OPERADORA (só 1ª linha)
                row_data[23] = ip_op if i == 0 else ""
                # Col 24: NETMASK OPERADORA (só 1ª linha)
                row_data[24] = mask_op if i == 0 else ""

                # Renderizar colunas não-merged
                for col in range(4, 25):
                    if col in (5,):
                        continue  # col 5 já merged (CN OP)
                ...
                # Bordas para as colunas merged (linhas 2+ precisam de borda)
                for mcol in (2, 3, 5):
                    ws.cell(row=r, column=mcol).border = s.box_border
                    ws.cell(row=r, column=mcol).fill = f
