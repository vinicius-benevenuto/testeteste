...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        '<span style="font-size:12px;color:#94A3B8;">Selecione uma coluna abaixo para ver o dashboard instantâneo:</span>'
        '</div>',
        unsafe_allow_html=True,
    )

    # ── Seletor de coluna (via botões inline — sem selectbox) ──────────────
    col_btns = st.columns(len(BUSINESS_COLS))
    for i, col in enumerate(BUSINESS_COLS):
        with col_btns[i]:
            is_sel = sel_col == col
            btn_style = (
                "background:#1B5FBF;color:#fff;border:none;"
                if is_sel else
                "background:#F8F9FA;color:#374151;border:1px solid #D1D5DB;"
            )
            if st.button(
                col,
                key=f"col_btn_{col}",
    ...
    if sel_col:
        ...
        # ── Exportação PPTX da coluna ──────────────────────────────────────
        _divider()
        ex1, ex2 = st.columns(2, gap="medium")
        with ex1:
            try:
                pptx_col = build_column_pptx(tf, sel_col)
                fname    = f"dashboard_{sel_col}_{datetime.now().strftime('%Y-%m-%d_%H%M')}.pptx"
                st.download_button(
                    f"Exportar dashboard — {sel_col} (PPTX)",
                    data=pptx_col,
                    file_name=fname,
        ...
        with ex2:
            try:
                ...
                fname_g  = f"dashboard_geral_{datetime.now().strftime('%Y-%m-%d_%H%M')}.pptx"
                st.download_button(
                    "Exportar dashboard geral (PPTX)",
                    data=pptx_gen,
                    file_name=fname_g,
                    mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                    key="btn_pptx_gen",
        ...
        # Apenas exportação geral (sem coluna selecionada)
        _divider()
        try:
            pptx_gen = build_general_pptx(tf)
            fname_g  = f"dashboard_geral_{datetime.now().strftime('%Y-%m-%d_%H%M')}.pptx"
