...
# Ajuste o caminho, delimitador e nomes de colunas conforme seu arquivo
CAMINHO_ANEXO5          = r"C:\Users\40418843\Downloads\Anexo5.txt"
DELIMITADOR_ANEXO5      = "\t"           # tab é o padrão do Anexo 5 da ANATEL
                                         # troque por ";" ou "|" se necessário
COLUNA_RN1_ANEXO5       = "RN1"          # coluna RN1 no Anexo 5
COLUNA_OPERADORA_ANEXO5 = "Nome Fantasia" # coluna com nome da operadora
...
def _ler_arquivo_em_chunks(caminho: str, chunksize: int, delimitador: str = ";", dtype=None):
    """
    Gerador de chunks. Para XLSX faz fatiamento manual (sem suporte nativo).
...
def carregar_portabilidade() -> dict:
    """
    Lê a base de portabilidade com delimitador '|' e constrói:
...
# ==============================================================================
# ETAPA 2b — ANEXO 5 (rn1 → Nome Fantasia)
# ==============================================================================
...
def carregar_anexo5() -> dict:
    ...
    df = _ler_arquivo_completo(
        CAMINHO_ANEXO5,
        delimitador=DELIMITADOR_ANEXO5,
        dtype=str,
    )
    ...
    col_rn1 = _normalizar_nome_coluna(COLUNA_RN1_ANEXO5)
    col_op  = _normalizar_nome_coluna(COLUNA_OPERADORA_ANEXO5)

    if col_rn1 not in df.columns:
        raise ValueError(
            f"Anexo 5: coluna RN1 '{col_rn1}' não encontrada.\n"
            f"Colunas disponíveis: {list(df.columns)}"
        )
    if col_op not in df.columns:
        raise ValueError(
            f"Anexo 5: coluna de operadora '{col_op}' não encontrada.\n"
            f"Colunas disponíveis: {list(df.columns)}"
        )

    df = df[[col_rn1, col_op]].dropna(subset=[col_rn1]).copy()
    df[col_rn1] = df[col_rn1].astype(str).str.strip()
    df[col_op]  = df[col_op].fillna("").astype(str).str.strip()

    idx_anexo5 = dict(zip(df[col_rn1], df[col_op]))
...
def normalizar_numero(numero: str) -> Optional[str]:
    ...
    Normaliza número brasileiro:
      ...
      2. Remove DDI '55' inicial (se len > 12 após remoção ficaria < 10).
