...
<style>
  ...
  .sbc-badge-atencao{ background:#ffe0b2; color:#7f4e00; }
  ...
  .sbc-cpu-bar.low{ background:#198754; }
  .sbc-cpu-bar.mid{ background:#ffc107; }
  .sbc-cpu-bar.high{ background:#fd7e14; }
  .sbc-cpu-bar.crit{ background:#dc3545; }
  .sbc-card-footer{
    display:flex; align-items:center; justify-content:space-between;
...
<script>
/* ===== GLOBALS ===== */
const engineerMode = {{ 'true' if engineer_mode else 'false' }};
const ROW_CAP = 10;
...
const SBC = (function(){
  ...
  function cpuBarClass(pct){
    if(pct<=50) return 'low'; if(pct<=75) return 'mid'; if(pct<=90) return 'high'; return 'crit';
  }
  function badgeClass(saude){ return 'sbc-badge-'+(saude||'disponivel'); }
  function saudeLabel(s){
    return {disponivel:'Disponível',moderado:'Moderado',atencao:'Atenção',critico:'Crítico'}[s]||s;
  }
  ...
  function renderCards(data){
    ...
    if(data.csv_source) metaEl.innerHTML = `Fonte: <strong>${data.csv_source}</strong> · ${data.csv_modificado_em||''}`;
    ...
    data.sbcs.forEach((sbc)=>{
      ...
      card.innerHTML = `
        <div class="sbc-card-header">
          <span class="sbc-card-name">${sbc.nome}</span>
          <div class="d-flex gap-1 align-items-center">
            <span class="sbc-card-badge ${badgeClass(sbc.saude)}">${saudeLabel(sbc.saude)}</span>
            ${recBadge}
          </div>
        </div>
        <div class="sbc-cpu-bar-wrap">
          <div class="sbc-cpu-bar ${cpuBarClass(sbc.cpu_max)}" style="width:${Math.min(100,sbc.cpu_max)}%"></div>
        </div>
        <div class="sbc-card-details">
          <span>Modelo: <strong>${sbc.modelo||'—'}</strong></span>
          <span>CPU máx: <strong>${sbc.cpu_max}%</strong> · média: ${sbc.cpu_media}%</span>
          <span>Serviços: ${(sbc.servicos||[]).join(', ')||'—'}</span>
          <span>Medições: ${sbc.total_medicoes} · Status: ${sbc.status_csv}</span>
        </div>
        <div class="sbc-card-footer">
          <span class="sbc-score">Score: ${sbc.score}/100</span>
