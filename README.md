...
function wireCN(tr){
  ...
  // Apenas eventos do usuário — sem auto-fire no load para não cancelar outras linhas
  ...
  rn1I.addEventListener('input',()=>{
    ...
    t=setTimeout(()=>{
      ...
      document.querySelectorAll('#tableVivo tbody tr').forEach((tr,ri)=>{
        ...
        if(cn.length>=2) setTimeout(()=>{ SIP.query(cn, ri); }, ri * 500);
      });
    },400);
  });
})();
...
(function(){
  ...
  // Consultar siprouter ao carregar — delay escalonado por linha para não cancelar o debounce
  setTimeout(()=>{
    ...
    document.querySelectorAll('#tableVivo tbody tr').forEach((tr,ri)=>{
      ...
      const cn=(cnI?.value||'').replace(/\D/g,'');
      if(cn.length===2){
        // Cada linha aguarda ri*500ms extra → evita que uma linha cancele o debounce da anterior
        setTimeout(()=>{ SIP.query(cn, ri); }, ri * 500);
      }
    });
  }, 400);
})();
