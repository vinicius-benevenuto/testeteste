...
def exportar_excel(form_id: int):
    ...
    # Resultado IPAM — lógica honesta
    tentativas = builder.ipam_tentativas
    falhas     = builder.ipam_falhas
    reservas   = builder.ipam_reservas

    if tentativas == 0:
        # Não tentou — mask ou CN não preenchidos
        session["_ipam_ok"]  = None   # None = não aplicável
        session["_ipam_msg"] = ""
    elif falhas:
        # Tentou mas falhou
        session["_ipam_ok"]  = False
        session["_ipam_msg"] = "; ".join(falhas)
    elif reservas:
        # Tentou e reservou com sucesso
        session["_ipam_ok"]  = True
        session["_ipam_msg"] = ""
    else:
        # Tentou mas sem resultado (ex: já reservado, subnet não encontrada)
        session["_ipam_ok"]  = False
