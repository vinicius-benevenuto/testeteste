...
class SBCMeasurement:
    ...
    cpu_pct: float
    cpu_max: float
    cpu_avg: float       # CPU AVG do arquivo — métrica primária de capacidade
    status: str
    servico: str
...
class SBCAnalysisResult:
    ...
    cpu_avg: float            # Média das leituras de CPU AVG — métrica primária
    cpu_media: float          # Média de todas as leituras de % CPU (legado)
    cpu_max: int              # Maior valor de CPU MAX entre todas as medições
    cpu_min: int              # Menor valor de % CPU (mostra a faixa)
    cpu_ultima: float         # Última leitura de CPU AVG (mais recente)
...
class SBCAnalyzer:
    ...
    def _classify_health(self, cpu_avg: float) -> str:
        """
        Classifica saúde do SBC baseado no CPU AVG (média de ocupação).
        Usa os thresholds configurados em SBC_HEALTH_THRESHOLDS.
        Quanto menor o AVG, mais vazio e disponível o SBC está.
        """
    ...
    def _calculate_score(self, cpu_avg: float, cpu_max: float,
        ...
        Fórmula:
          base = 100 - cpu_avg (quanto menor o AVG, mais vazio, melhor)
          ...
          - penalidade de CPU MAX muito alta (>85%: -5)
        """
        ...
        if cpu_max > 85:
            penalty += 5  # Pico de CPU muito alto é preocupante
    ...
    def _generate_reason(self, result: SBCAnalysisResult) -> str:
        ...
        if result.saude == "disponivel":
            parts.append(f"CPU avg {result.cpu_avg}% — capacidade ampla")
        elif result.saude == "moderado":
            parts.append(f"CPU avg {result.cpu_avg}% — ocupação moderada")
        elif result.saude == "atencao":
            parts.append(f"CPU avg {result.cpu_avg}% — ocupação alta, monitorar")
        else:
            parts.append(f"CPU avg {result.cpu_avg}% — capacidade crítica")
    ...
    def _aggregate_sbc_measurements(self, measurements: list[SBCMeasurement]) -> list[SBCAnalysisResult]:
        ...
        - cpu_avg: média das leituras de CPU AVG — MÉTRICA PRIMÁRIA de capacidade
        - cpu_media: média aritmética de % CPU (legado, mantido para compatibilidade)
        ...
        Se a coluna CPU AVG não existir no arquivo (todos 0), usa cpu_pct como fallback.
        """
        ...
        for sbc_name, sbc_measurements in groups.items():
            # CPU AVG: métrica primária (com fallback para cpu_pct se AVG não existir)
            ...
            # Se CPU AVG está todo zerado, usa cpu_pct como fallback
            ...
            # Classificação de saúde e score — baseados em CPU AVG
            saude = self._classify_health(cpu_avg_media)
            score = self._calculate_score(
                cpu_avg_media, cpu_max, worst_status, len(servicos), len(sbc_measurements)
            )
            ...
            result = SBCAnalysisResult(
                ...
                regional=regional,
                uf=uf,
                cpu_avg=round(cpu_avg_media, 1),
                cpu_media=round(cpu_pct_media, 1),
                cpu_max=int(cpu_max),
                cpu_min=int(cpu_min),
        ...
        # Ordena por score (maior primeiro) — SBCs mais vazios ficam no topo
...
class PTIWorkbookBuilder:
    ...
    def __init__(self, form_row):
        ...
        # Dados do formulário para preenchimento dinâmico do Excel
    ...
    def _extract_vivo_rows(self):
        """Extrai linhas de dados_vivo_json (tabela Dados VIVO do formulário)."""
    ...
    def _extract_op_rows(self):
        """Extrai linhas de dados_operadora_json (tabela Dados Operadora do formulário)."""
    ...
    def _unique_field(self, rows, field):
        """Extrai valores únicos não-vazios de um campo, preservando ordem."""
        seen = []
        for r in rows:
            v = r.get(field, "").strip()
    ...
    def _b_versions(self):
        ...
        # Dados da primeira linha: Data, Resp Eng, Resp Atacado, Escopo, CN, Áreas Locais
        fd={
            4: self.created_str,
            5: self.resp_eng,
            6: self.resp_atk,
    ...
    def _b_diagram(self):
        ...
        # Cada bloco usa 12 colunas (3-14), blocos adicionais deslocam +12 colunas
        block_width = 12
        total_cols = 2 + (num_blocks * block_width)
        # Largura das colunas: col 1 margem, depois 12 colunas por bloco, última margem
        col_widths = {1: 2.0}
        ...
        # Cabeçalho global (merge por todos os blocos)
        ...
        # Cada bloco: CN header, Ponta A (VIVO), Ponta B (Operadora), tabelas Tráfego/IP/MASK
        traffic_items = [t for t in self.traffic if t]  # Caixinhas de escopo preenchidas
        if not traffic_items:
            traffic_items = [""]  # Pelo menos uma linha vazia

        for b_idx in range(num_blocks):
            base_col = 3 + (b_idx * block_width)
            vivo_row = self.vivo_rows[b_idx] if b_idx < len(self.vivo_rows) else {}
            # Buscar linha da operadora correspondente (pelo mesmo CN ou índice)
            ...
            mid = base_col + 5  # Ponta B começa 5 colunas depois de base_col
            ...
            # Linhas 21+: Dados de tráfego (uma linha por caixinha de escopo)
            num_traffic = max(len(traffic_items), 3)  # Mínimo 3 linhas
            for j in range(num_traffic):
                ...
                # Ponta A (VIVO): Tráfego, Endereço IP, NET MASK
                ws.cell(row=r_row, column=base_col, value=traf_val).fill = f
                ws.cell(row=r_row, column=base_col).font = s.font_small
                ws.cell(row=r_row, column=base_col+1, value=endereco_vivo if j == 0 else "").fill = f
                ws.cell(row=r_row, column=base_col+1).font = s.font_small
                ws.cell(row=r_row, column=base_col+2, value=mask_val).fill = f
                ws.cell(row=r_row, column=base_col+2).font = s.font_small

                # Ponta B (Operadora): Tráfego, Endereço IP, NET MASK
                ws.cell(row=r_row, column=mid+1, value=traf_val).fill = f
                ws.cell(row=r_row, column=mid+1).font = s.font_small
                ws.cell(row=r_row, column=mid+2, value=faixa_ip_op if j == 0 else "").fill = f
                ws.cell(row=r_row, column=mid+2).font = s.font_small
                ws.cell(row=r_row, column=mid+3, value="").fill = f
                ws.cell(row=r_row, column=mid+3).font = s.font_small
        ...
        img_row = 21 + max(len(traffic_items), 3) + 4
