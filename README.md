...
<style>
  ...
  /* --- Critical card distinct styling --- */
  .sbc-card.sbc-critico{
    border-color:#dc3545; border-width:2px; border-style:dashed;
  ...
  .sbc-section-divider{
    ...
    margin:.6rem 0 .4rem; padding:0; font-size:.78rem; font-weight:700;
  }
  .sbc-section-divider::before,.sbc-section-divider::after{
  ...
  .sbc-summary-bar{
    ...
    font-size:.78rem; margin-bottom:.5rem; padding:.35rem .5rem;
    background:#f8f9fa; border-radius:.4rem; border:1px solid #e9ecef;
  }
  .sbc-summary-chip{
    display:inline-flex; align-items:center; gap:.25rem;
    padding:.15rem .45rem; border-radius:999px; font-weight:600; font-size:.72rem;
  }
  ...
  .eng-grid-card-title{
    ...
    background:var(--vh-primary,#6b09a6); color:#fff;
    height:42px; display:flex; align-items:center;
    flex-shrink:0;
  }
  .eng-grid-card-body{ padding:.4rem .5rem; flex:1; }
  .eng-grid-card-body .row-line{
    display:grid; grid-template-columns:28px 1fr;
    margin-bottom:.25rem; align-items:start;
  }
  .eng-grid-card-body .row-line .form-check-input{
  ...
  >
    ...
    <section class="card section-card mb-3" id="eng-section" aria-labelledby="sec-eng">
      ...
      <div class="card-body">
        ...
        <div class="group-title fw-semibold mt-1 mb-2">Observações da Engenharia (opcional)</div>
...
const SBC = (function(){
  ...
  /* === Renderizar cards — TODOS os SBCs, agrupados visualmente === */
  function renderCards(data){
    ...
    const nDisp = data.total_disponiveis || disponiveis.length;
    const nCrit = data.total_criticos || criticos.length;

    /* Barra resumo */
    const summaryBar = document.createElement('div');
    summaryBar.className = 'sbc-summary-bar';
    summaryBar.innerHTML = `
      <span style="font-weight:600">${data.sbcs.length} SBC(s) encontrado(s)</span>
      ${nDisp>0 ? `<span class="sbc-summary-chip disp">✔ ${nDisp} disponível(is)</span>` : ''}
      ${nCrit>0 ? `<span class="sbc-summary-chip crit">⚠ ${nCrit} crítico(s)</span>` : ''}
    `;
    listEl.appendChild(summaryBar);

    /* Renderizar grupo disponíveis */
    if(disponiveis.length>0){
      const divD = document.createElement('div');
      divD.className = 'sbc-section-divider disp-divider';
      divD.innerHTML = `✔ Disponíveis (${disponiveis.length})`;
      listEl.appendChild(divD);
      disponiveis.forEach(sbc => listEl.appendChild(buildCard(sbc, false, uf)));
    }

    /* Renderizar grupo críticos */
    if(criticos.length>0){
      const divC = document.createElement('div');
      divC.className = 'sbc-section-divider crit-divider';
      divC.innerHTML = `⚠ Críticos (${criticos.length})`;
      listEl.appendChild(divC);
  ...
  function buildCard(sbc, isCritico, uf){
    ...
    const recBadge = sbc.recomendado && !isCritico
      ? '<span class="sbc-card-badge sbc-badge-disponivel">✅ RECOMENDADO</span>'
      : '';
    const critWarn = isCritico
      ? '<span class="sbc-card-badge" style="background:#dc3545;color:#fff;font-size:.65rem">⚠ CRÍTICO</span>'
      : '';
    ...
    /* Botão — texto diferente para crítico */
    ...
    card.innerHTML = `
      ...
      <div class="sbc-card-footer">
        ...
        <button type="button" class="btn btn-sm ${btnClass} sbc-use-btn" data-sbc-name="${sbcNome}" data-critico="${isCritico?'1':'0'}">
          ${btnLabel}
        </button>
      </div>`;

    /* Evento do botão com confirmação para críticos */
    card.querySelector('.sbc-use-btn').addEventListener('click', function(){
      const nome = this.dataset.sbcName;
      const crit = this.dataset.critico === '1';
      if(crit){
        if(!confirm(`⚠️ ATENÇÃO: O SBC "${nome}" está em estado CRÍTICO.\n\nDeseja usá-lo mesmo assim?`)){
          return;
        }
      }
...
function wireCnAutoFill(tr){
  ...
  /* Debounce: impede que input+change+blur disparem 3x seguidas */
  let _cnTimer = null;
  let _ufTimer = null;
  ...
  cnInp.addEventListener('input', cnHandler);
  ...
  ufInp.addEventListener('input', ufHandler);
...
form.addEventListener('submit',(e)=>{
  ...
  if(engineerMode){
    ...
    /* Validação obrigatória: cada grupo deve ter ao menos 1 checkbox marcado */
    const groups = document.querySelectorAll('#eng-section .eng-grid-card[data-eng-group]');
    let engValid = true;
    groups.forEach(card => {
      const checks = card.querySelectorAll('.eng-flag:checked');
      card.classList.remove('eng-invalid');
      if(checks.length === 0){
        card.classList.add('eng-invalid');
        engValid = false;
      }
    });
    if(!engValid){
      ...
      const first = document.querySelector('#eng-section .eng-grid-card.eng-invalid');
