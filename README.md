        ...
        with st.spinner("Processando..."):
            try:
                ...
                with session_scope() as s:
                    ...
                    # Coleta CNLs do Portal (CNL_PPI) para resolver UF
                    _cnl_por_col = cfg.get("cnl_por_col", "CNL_PPI")
                    _cnl_sci_col = cfg.get("cnl_sci_col", "CNL")
                    _cnls_por = list(por_df.get(_cnl_por_col, pd.Series()).dropna()) if por_df is not None else []
                    _cnls_sci = list(sci_df.get(_cnl_sci_col, pd.Series()).dropna()) if sci_df is not None else []
                    all_cnls = [clean_cnl(c) for c in _cnls_por + _cnls_sci if clean_cnl(c)]
                    uf_map       = repo.resolve_ufs_batch(all_cnls)
