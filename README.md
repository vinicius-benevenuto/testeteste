...
class PTIWorkbookBuilder:
    ...
    def reserva_redes_ipam(self, vivo_rows: list[dict]) -> None:
        ...
        for item in vivo_rows:
            ...
            descricao  = item.get("escopo", "")
            if not cn_usuario or not mask_alvo:
                continue
            nome_pool = IPAM_MASK_POOL_MAP.get(mask_alvo, IPAM_DEFAULT_POOL)
            logger.info("Reserva IPAM | CN: %s | /%s | Pool: %s", cn_usuario, mask_alvo, nome_pool)
    ...
    def _b_diagram(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            # SBCs
            r = cursor
            for lc, sbc in [(LC, sbc_vivo), (MID, sbc_op)]:
                ws.merge_cells(start_row=r, start_column=lc, end_row=r, end_column=lc+2)
                ws.cell(row=r, column=lc, value=f"SBC: {sbc}" if sbc else "SBC: ___").font = Font(name="Calibri", size=9, bold=True)
            ...
            for j in range(num_traffic):
                ...
                for lc_start, ip_val in [(LC, endereco_vivo), (MID, faixa_ip_op)]:
                    ...
                    ip_c = ws.cell(row=r, column=lc_start+1, value="" if num_visible > 1 else ip_val)
                    ip_c.font = s.font_small; ip_c.fill = f; ip_c.alignment = s.align_center; ip_c.border = s.box_border
                    mk_c = ws.cell(row=r, column=lc_start+2, value="" if num_visible > 1 else mask_val)
                    mk_c.font = s.font_small; mk_c.fill = f; mk_c.alignment = s.align_center; mk_c.border = s.box_border
                _gap(r, LC+3, MID)

            if num_visible > 1:
                last_vis = data_start_row + num_visible - 1
                for lc_start, ip_val in [(LC, endereco_vivo), (MID, faixa_ip_op)]:
                    for offset, val in [(1, ip_val), (2, mask_val)]:
                        ws.merge_cells(start_row=data_start_row, start_column=lc_start+offset,
                                       end_row=last_vis, end_column=lc_start+offset)
                        mc = ws.cell(row=data_start_row, column=lc_start+offset, value=val)
                        mc.font = s.font_small; mc.alignment = s.align_center; mc.border = s.box_border
    ...
    def _b_routing(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            if not sbc_vivo:     missing.append("SBC")
            if not area_local:   missing.append("ÁREA LOCAL")
            if not endereco_vivo: missing.append("IP DA VIVO")
    ...
    def _b_conc(self) -> None:
        ...
        aw = Alignment(horizontal="center", vertical="center", wrap_text=True)
        self._bh(ws, 2, 2, last_col)
        self._st(ws, 4, 2, last_col, "2.5 - Concentração")

        for row, text, italic in [
            (5, f"A '{n.upper()}' informou interesse em realizar a concentração de ALs conforme detalhado na tabela abaixo:", False),
            (6, 'Caso necessite de incluir mais ALs, inserir linhas adicionais (coluna B - "Ref").', True),
        ]:
        ...
        for col, txt in [(2,"Ref"),(3,"CN"),(4,"CNL"),(5,"Cod. CnI"),(6,"Status Link")]:
            c = ws.cell(row=hr, column=col, value=txt)
            c.font = Font(name="Calibri", size=10, bold=True, color="FFFFFF")
            c.alignment = aw; c.fill = s.fill_secondary; c.border = s.box_border
        ...
        for i, d in enumerate(conc_rows):
            ...
            for col_num, key in [(2,"ref"),(3,"cn"),(4,"cnl"),(5,"cod_cni"),(6,"status_link")]:
                val = d.get(key, "")
                c = ws.cell(row=r, column=col_num, value=val)
                c.font = s.font_body; c.fill = f; c.border = s.box_border
        ...
        for col_num, w in max_w.items():
            ws.column_dimensions[get_column_letter(col_num)].width = max(col_w.get(col_num, 8), w)
    ...
    def _b_mtl(self) -> None:
        ...
        for ref_i in range(num_refs):
            ...
            for row_data in [
                {3:cv, 4:"", 5:n.upper(), 6:ev, 7:eo, 8:"", 9:"", 10:"", 11:"", 12:""},
                {3:co or cv, 4:"", 5:"VIVO", 6:ev, 7:eo, 8:"", 9:"", 10:"", 11:"", 12:""},
            ]:
    ...
    def build(self) -> Workbook:
        ...
        self._b_diagram()
        self._b_routing()
        self._b_conc()        # estava ausente no código original
        self._b_prefixes()
        self._b_mtl()
        self._b_params()
