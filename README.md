  ...
  >
    ...
    <div class="sec">
      ...
      <div class="sec-body" style="padding:.75rem">
        ...
        <!-- Painel Siprouter -->
        <div id="sipPanel" style="display:none;margin-top:.75rem;border:1px solid #d4c5f9;border-radius:10px;background:linear-gradient(135deg,#faf8ff,#fff);padding:.85rem 1rem">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem">
            <span style="font-size:.82rem;font-weight:700;color:var(--p)">
              <i class="bi bi-router"></i> CDSIP_SPO_PL / CDSIP_SPO_JG
            </span>
            <span id="sipPanelMeta" style="font-size:.7rem;color:var(--sub)"></span>
          </div>
...
const SIP = (function(){
  ...
  function renderResult(data, rowIdx){
    resultEl.innerHTML=''; loadEl.style.display='none'; emptyEl.style.display='none';
    ...
    metaEl.textContent = `CN ${data.cn} · RN1 ${data.rn1_consultado} · ${data.resultados.length} registro(s)`;

    // Montar tabela de resultados
    const tbl = document.createElement('table');
    tbl.style.cssText='width:100%;border-collapse:collapse;font-size:.75rem';
    tbl.innerHTML=`<thead><tr style="background:#f9fafb;border-bottom:1px solid var(--bdr)">
      <th style="padding:.35rem .6rem;font-weight:600;color:var(--sub);font-size:.68rem">Descrição</th>
      <th style="padding:.35rem .6rem;font-weight:600;color:var(--sub);font-size:.68rem">VLAN</th>
      <th style="padding:.35rem .6rem;font-weight:600;color:var(--sub);font-size:.68rem;white-space:nowrap">CDSIP_SPO_PL</th>
      <th style="padding:.35rem .6rem;font-weight:600;color:var(--sub);font-size:.68rem;white-space:nowrap">CDSIP_SPO_JG</th>
      <th style="padding:.35rem .6rem"></th>
    </tr></thead>`;
    ...
    data.resultados.forEach((r, i)=>{
      ...
      const blocoLabel = [r.pl_bloco?`PL: ${r.pl_bloco}`:'',r.jg_bloco?`JG: ${r.jg_bloco}`:''].filter(Boolean).join(' / ');
      tr.innerHTML=`
        <td style="padding:.35rem .6rem;font-weight:500">${r.descricao||'—'}</td>
        <td style="padding:.35rem .6rem;color:var(--sub)">${r.vlan||'—'}</td>
        <td style="padding:.35rem .6rem;font-family:monospace;font-size:.72rem">${r.pl_bloco||'—'}</td>
        <td style="padding:.35rem .6rem;font-family:monospace;font-size:.72rem">${r.jg_bloco||'—'}</td>
        <td style="padding:.35rem .6rem;text-align:right">
          <button type="button" class="btn-g btn-sm" style="font-size:.68rem;padding:.2rem .5rem"
                  data-bloco="${blocoLabel}" data-rowIdx="${rowIdx}">
            Usar
          </button>
        </td>`;
    ...
    // Bind botões "Usar"
    resultEl.querySelectorAll('button[data-bloco]').forEach(btn=>{
      btn.addEventListener('click',()=>{
        const ri = parseInt(btn.dataset.rowIdx, 10);
        fillBlocoIP(btn.dataset.bloco, ri);
      });
    });
