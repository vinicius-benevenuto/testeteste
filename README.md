...
def render_upload_page() -> None:
    ...
    st.info(f"Seeds: {cnl_n:,} registros CNL · {uf_n} mapeamentos CN→UF")
    if cnl_n == 0:
        st.warning("Tabela CNL vazia. Execute: **Ferramentas → Carregar Seeds** para habilitar derivação de UF.")
...
def render_mapping_page() -> None:
    ...
    if sci_df is None or por_df is None:
        st.warning("Carregue Science e Portal primeiro (Passo 1).")
        return
...
def render_merge_page() -> None:
    ...
    if sci_df is None or por_df is None:
        st.warning("Carregue os arquivos primeiro (Passo 1).")
        return
    ...
    if not cfg_saved:
        st.warning("Configuração não salva. Vá para **2. Mapeamento** e clique **Salvar**.")
    else:
        ...
        if id_rota in ("(não definida)", "(nenhuma)", ""):
            st.warning("ID Rota não configurada** — Etapa 3 do Science será ignorada. "
    ...
    with st.expander("📋 Relatório Completo do Pipeline", expanded=True):
        ...
        # ── Correções automáticas (Regras 1, 2 e 3) ──────────────────
        q1, q2, q3, q4 = st.columns(4)
        q1.metric("Correções de REDE (R1)",
                  report.get("correcoes_rede", 0),
                  delta_color="inverse" if report.get("correcoes_rede", 0) > 0 else "off",
                  help="Linhas onde REDE foi corrigida automaticamente (origem → REDE imutável)")
        q2.metric("Tipo de Rota preenchido (R2)",
                  report.get("correcoes_tipo_rota", 0),
                  help="Linhas com Tipo de Rota vazio preenchido com ITX-SIP_AS")
        q3.metric("UF via CNL_PPI (R3 ✅)",
                  report.get("uf_preenchido_r3", 0),
                  help="Registros Portal com UF derivada com sucesso via CNL_PPI → CN → UF")
        q4.metric("UF não derivada (R3)",
                  report.get("uf_invalido_r3", 0),
                  delta_color="inverse" if report.get("uf_invalido_r3", 0) > 0 else "off",
                  help="Registros Portal onde CNL_PPI era inválido ou CN não encontrado na tabela")

        if report.get("uf_divergente_r3", 0) > 0:
            st.warning(
                f"**{report['uf_divergente_r3']} registro(s) com UF divergente** entre "
        ...
        if quality_logs:
            with st.expander(f"Log de Correções Automáticas ({len(quality_logs)})", expanded=False):
                df_ql = pd.DataFrame(quality_logs)
                cols_show = [c for c in ["regra", "rotulo", "antes", "depois", "mensagem"] if c in df_ql.columns]
        ...
        if all_incons:
            st.markdown(f"#### Inconsistências Detalhadas ({len(all_incons)})")
    ...
    with ex1:
        if st.button("Salvar no SQLite", key="btn_save"):
            _save_to_db(merged_df)

    with ex2:
        st.download_button("CSV (filtrado)",
                           to_csv_bytes(filtered_df),
                           f"resultado_{version_tag()}.csv",
                           "text/csv", key="btn_csv")
    with ex3:
        try:
            st.download_button("XLSX (filtrado)",
...
def render_validation_page() -> None:
    ...
    if not uf_pending.empty:
        with st.expander(f"Linhas sem UF ({len(uf_pending)})"):
...
def render_diagnostico_page() -> None:
    ...
    if por_df is not None:
        ...
        if centrais_sem_mapa:
            st.warning(f"{len(centrais_sem_mapa)} valores de CENTRAL sem mapeamento: "
...
def render_logs_page() -> None:
    ...
    st.dataframe(df_l[df_l["level"].isin(lvl_filter)],
                 width="stretch", hide_index=True)
    st.download_button("Baixar log", logs_to_text(logs),
