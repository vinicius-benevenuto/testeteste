...
{% block title %}{{ 'Validar PTI' if engineer_mode else 'Pré-PTI' }} — PTI AUTOMATIZADO{% endblock %}
...
<style>
  ...
  /* Mask vazio — roxo discreto (deve vir DEPOIS das exceções para sobrescrever) */
  .eng-mask.mask-empty {
      ...
      <div class="sec-body">
        ...
        <div style="margin-top:.9rem">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:.6rem">
            {% for val, lbl in [('LC','LC'),('LD15 + CNG','LD15 + CNG'),('LDS/CSP + CNG','LDS/CSP + CNG'),('Transporte','Transporte'),('VC1','VC1')] %}
...
const SIP = (function(){
  ...
  const _timers = {};   // timer por linha: {rowIdx: timerId}
  const _last   = {};   // cache por linha: {rowIdx: cacheKey}

  function resetCache(){
    Object.keys(_last).forEach(k => delete _last[k]);
  ...
  async function query(cn, rowIdx){
    ...
    const ck = `${cn}:${rn1}:${scm}:${av}`;
    if(_last[rowIdx] === ck) return;
    _last[rowIdx] = ck;

    clearTimeout(_timers[rowIdx]);
    _timers[rowIdx] = setTimeout(async()=>{
      panel.style.display='block';
...
function wireCN(tr){
  ...
  // Auto-fire seguro: cada linha tem seu próprio timer independente
...
(function(){
  ...
  function addOp(v={}){
    ...
    // SBC auto-preenchido como "SBC." + EOT LC — usuário não digita
    (function(){
      const cells = tr.querySelectorAll('td');
      const iEot  = cells[2]?.querySelector('input');  // eto_lc = posição 2
      const iSbc  = cells[5]?.querySelector('input');  // sbc    = posição 5
      if(iSbc){
        iSbc.setAttribute('readonly','');
        iSbc.style.background = '#f9fafb';
        iSbc.style.color = 'var(--sub)';
      }
      function autoSbc(){
        const eot = (iEot?.value||'').trim();
        if(iSbc) iSbc.value = eot ? `SBC.${eot}` : '';
        setDirty();
      }
      iEot?.addEventListener('input',  autoSbc);
      iEot?.addEventListener('change', autoSbc);
      iEot?.addEventListener('blur',   autoSbc);
      // Preencher imediatamente se EOT LC já tem valor (load de dados salvos)
      autoSbc();
    })();
    ...
    if(ENG){
      ...
      // Cor roxo via setProperty('important') — vence o !important do CSS
      const updateMaskColor = () => {
        if((iMask.value||'').trim()){
          iMask.style.removeProperty('background');
        } else {
          iMask.style.setProperty('background', '#ede9fe', 'important');
        }
      };
      updateMaskColor();
      iMask.addEventListener('input', updateMaskColor);
    } else {
  ...
  // Garantia de preenchimento no load: re-consultar linhas com CN já preenchido
  setTimeout(()=>{
...
/* ── Destaque do campo Mask vazio no modo Engenharia ── */
if(ENG){
  const markMask = () => {
    document.querySelectorAll('.eng-mask').forEach(el => {
      el.classList.toggle('mask-empty', !(el.value || '').trim());
    });
  };
  markMask();
  setTimeout(markMask, 800);
  // Re-verificar quando o usuário preenche
  document.addEventListener('input', e => {
    if(e.target.classList.contains('eng-mask')) markMask();
  });
}
</script>
{% endblock %}
