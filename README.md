...
class SBCAnalyzer:
    ...
    def _classify_health(self, status: str) -> str:
        ...
        return SBC_STATUS_TO_HEALTH.get(key, "moderado")
    ...
    def _generate_reason(self, result: SBCAnalysisResult) -> str:
        ...
        parts.append(f"Status: {result.status_fonte} — {labels.get(result.saude, '')}")
        parts.append(f"CAPS avg {result.caps_avg} (mín {result.caps_min}, máx {result.caps_max})")
        if result.caps_tendencia != "estavel":
