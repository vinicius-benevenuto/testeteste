...
class SBCAnalyzer:
    ...
    def suggest_for_cn(self, cn: str, confirmar_criticos: bool = False) -> SBCSuggestionResponse:
        ...
        cidade, uf = meta
        result = self.suggest_for_uf(uf, confirmar_criticos=confirmar_criticos)
        # Enriquecer com CN e cidade originais
        result.cn = cn
        result.cidade = cidade
        return result

    def suggest_for_uf(self, uf: str, confirmar_criticos: bool = False) -> SBCSuggestionResponse:
        """
        Busca SBCs pela UF com lógica de classificação crítico/não-crítico.

        Fluxo:
          1. Filtra todos os SBCs do UF informado.
          2. Classifica em críticos vs não-críticos.
          3. Se existem não-críticos → sugere APENAS esses.
          4. Se TODOS são críticos → sinaliza todos_criticos=True.
             - Se confirmar_criticos=True (usuário confirmou) → retorna os críticos.
             - Se False → retorna aguardando_confirmacao=True + fallback regional.
          5. Se não existem SBCs → informa e busca vizinhos/regional.
        """
        uf = uf.strip().upper()
        if len(uf) != 2:
            return SBCSuggestionResponse(
        ...
        # === PASSO 1: Filtrar SBCs do UF ===
        uf_measurements = [m for m in measurements if m.uf == uf]

        # === PASSO 5: Se não existem SBCs para o UF ===
            ...
            for neighbor_uf in neighbors:
                ...
                if uf_measurements:
                    ...
                    return SBCSuggestionResponse(
                        ...
                        mensagem=f"Nenhum SBC encontrado para {uf}. "
                                 f"Sugerindo {len(results)} SBC(s) de {neighbor_uf} (vizinha).",
                    )
            # Busca regional
            if regional:
                regional_ufs = [u for u, r in UF_TO_REGIONAL.items() if r == regional]
                reg_measurements = [m for m in measurements if m.uf in regional_ufs]
                if reg_measurements:
                    results = self._aggregate_sbc_measurements(reg_measurements)
                    data_medicao = results[0].dia_mais_recente if results else ""
                    return SBCSuggestionResponse(
                        cn=cn_found, uf=uf, cidade=cidade, regional=regional,
                        source_file=file_info.get("filename", ""),
                        ...
                        mensagem=f"Nenhum SBC encontrado para {uf}. "
                                 f"Sugerindo {len(results)} SBC(s) da regional {regional}.",
                    )
            return SBCSuggestionResponse(
                cn=cn_found, uf=uf, cidade=cidade, regional=regional,
                source_file=file_info.get("filename", ""),
        ...
        # === PASSO 2: Classificar críticos vs não-críticos ===
        ...
        # === PASSO 3: Se existem não-críticos → sugerir APENAS esses ===
        if nao_criticos:
            return SBCSuggestionResponse(
                cn=cn_found, uf=uf, cidade=cidade, regional=regional,
                source_file=file_info.get("filename", ""),
                source_modificado_em=file_info.get("modified_at", ""),
                data_medicao=data_medicao,
                total_sbcs=len(nao_criticos),
                sbcs=[asdict(r) for r in nao_criticos],
                fallback_usado=False, fallback_origem="",
                mensagem=f"{len(nao_criticos)} SBC(s) disponível(is) para {uf} ({cidade}). "
                         f"{len(criticos)} crítico(s) filtrado(s).",
            )

        # === PASSO 4: TODOS são críticos ===
        if confirmar_criticos:
            # Usuário confirmou → retornar os críticos do UF
            return SBCSuggestionResponse(
                cn=cn_found, uf=uf, cidade=cidade, regional=regional,
                source_file=file_info.get("filename", ""),
                source_modificado_em=file_info.get("modified_at", ""),
                data_medicao=data_medicao,
                total_sbcs=len(criticos),
                sbcs=[asdict(r) for r in criticos],
                fallback_usado=False, fallback_origem="",
                todos_criticos=True, aguardando_confirmacao=False,
                mensagem=f"⚠️ Todos os {len(criticos)} SBC(s) de {uf} estão CRÍTICOS. "
                         f"Exibindo conforme confirmação do usuário.",
            )

        # Usuário NÃO confirmou → sinalizar + sugerir alternativas regionais
        alternativas = []
        # Buscar SBCs não-críticos de vizinhos/regional
        neighbors = UF_NEIGHBORS.get(uf, [])
        for neighbor_uf in neighbors:
            n_meas = [m for m in measurements if m.uf == neighbor_uf]
            if n_meas:
                n_results = self._aggregate_sbc_measurements(n_meas)
                n_disponiveis = [r for r in n_results if r.saude != "critico"]
                alternativas.extend(n_disponiveis)
        if not alternativas and regional:
            regional_ufs = [u for u, r in UF_TO_REGIONAL.items()
                            if r == regional and u != uf]
            reg_meas = [m for m in measurements if m.uf in regional_ufs]
            if reg_meas:
                reg_results = self._aggregate_sbc_measurements(reg_meas)
                alternativas = [r for r in reg_results if r.saude != "critico"]

        # Montar resposta
        fallback_msg = ""
        if alternativas:
            alternativas.sort(key=lambda r: r.score, reverse=True)
            for i, a in enumerate(alternativas):
                a.recomendado = (i == 0)
            fallback_msg = (f" Sugerindo {len(alternativas)} alternativa(s) "
                           f"não-crítica(s) de regiões próximas.")
        ...
        return SBCSuggestionResponse(
            ...
            source_modificado_em=file_info.get("modified_at", ""),
            data_medicao=data_medicao,
            total_sbcs=len(alternativas),
            sbcs=[asdict(r) for r in alternativas],
            fallback_usado=bool(alternativas),
            fallback_origem="Regiões próximas (não-críticos)" if alternativas else "",
            todos_criticos=True,
            aguardando_confirmacao=True,
            mensagem=f"⚠️ Todos os {len(criticos)} SBC(s) de {uf} ({cidade}) "
                     f"estão CRÍTICOS.{fallback_msg}",
        )
...
def _register_routes(app):
    ...
    def api_sbc_suggest():
        ...
        confirmar = request.args.get("confirmar_criticos", "0").strip() in ("1", "true", "True")
        analyzer: SBCAnalyzer = app.config["SBC_ANALYZER"]
        if cn:
            result = analyzer.suggest_for_cn(cn, confirmar_criticos=confirmar)
        elif uf:
            result = analyzer.suggest_for_uf(uf, confirmar_criticos=confirmar)
        else:
        ...
        return jsonify(asdict(result) if hasattr(result, '__dataclass_fields__') else {
            ...
            "aguardando_confirmacao": result.aguardando_confirmacao,
        })
