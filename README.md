...
class PTIWorkbookBuilder:
    ...
    def __init__(self, form_row, sbc_analyzer=None, all_versions=None) -> None:
        ...
        self.ipam_falhas:   list[str] = []   # mensagens de erro para exibir ao usuário
    ...
    def _b_versions(self) -> None:
        ...
        for v_idx, v_form in enumerate(self.all_versions):
            ...
            from helpers import parse_db_datetime
            ...
            try:
                flags = _json.loads(row_get(v_form, "escopo_flags_json", "[]") or "[]")
                if flags:
            ...
            # CNs desta versão
            try:
                vr = _json.loads(row_get(v_form, "dados_vivo_json", "[]") or "[]")
                or_ = _json.loads(row_get(v_form, "dados_operadora_json", "[]") or "[]")
    ...
    def _b_diagram(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            _, cn_uf = cn_meta  # uf mantido para Encaminhamento, cidade não usada
    ...
    def _b_routing(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            # CN_METADATA removido — cidade não é mais vinculada ao CN

            # Separador bloco — apenas CN, sem cidade
            r = cursor
            ...
            # Colunas 21-24 preenchidas por linha; netmasks mescladas como no diagrama
            merged_cols = {
            ...
            # Mesclar netmasks (cols 22 e 24) para todos os tráfegos — uma célula única
            if DATA_ROWS > 1:
