...
def excel_download():
    import base64
    b64   = session.pop("_excel_b64",  None)
...
def exportar_excel(form_id: int):
    ...
    # Flash não aparece em send_file direto — salvar arquivo na sessão temporária
    # e redirecionar para página que exibe flash e faz download via JS
    import base64
    session["_excel_b64"]  = base64.b64encode(buf.read()).decode()
    session["_excel_name"] = f"PTI_{nome}_v{ver}_ID{form['id']}.xlsx"
    return redirect(url_for("engenharia.excel_download"))
