...
(function(){
  ...
  function addOp(v={}){
    ...
    if(ENG){
      ...
      // Aplicar cor imediatamente se vazio
      if(!(iMask.value||'').trim()) iMask.classList.add('mask-empty');
      iMask.addEventListener('input', ()=>{
        iMask.classList.toggle('mask-empty', !(iMask.value||'').trim());
      });
    } else {
