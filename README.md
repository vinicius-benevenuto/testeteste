...
IPAM_PASS     = os.getenv("IPAM_PASS", "230581Bs.@@")  # em prod: não usar default
...
class IPAMClient:
    ...
    def url_subnets_section(self) -> str:
        return f"{self.base_url}/api/ctp/sections/186/subnets/"
 ...
 # ------------------------------------------------------------------
    ...
    def buscar_id_cn(self, nome_cn: str) -> int:
        # Normaliza entrada (ex: "02", "2" → "CN 02")
    ...
    def buscar_rede_pai(
        ...
        mascara: str,
            ...
            logger.warning(
                "Rede pai /%s não localizada no pool ID %d. Prosseguindo.",
                mascara, id_pool,
            )
            return None
    ...
    def _b_prefixes(self) -> None:
        ...
        for i, (label, tipo, valor) in enumerate(indicators):
            ...
            if tipo == "binary":
                ...
                cr_cell.font = font_flag_ok if valor else font_flag_nok
            else:
                if valor and str(valor).strip():
                    cr_cell.value = str(valor).strip()
                    cr_cell.font = font_text_val
                else:
                    cr_cell.value = "✘"
