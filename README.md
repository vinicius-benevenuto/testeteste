...
/* Botão Reiniciar — estilo diferente dos nav buttons */
...
/* ════════════════════════════════════════════════════════════════════════════
   OCULTA O "Drag and drop" DO components.html NO PRIMEIRO RENDER
   
   Causa: components.v1.html() usa stComponentContainer como wrapper.
   Antes do iframe terminar de carregar, o Streamlit injeta um placeholder
   que visualmente parece um file uploader ("Drag and drop file here").
   
   Fix: esconder dropzone SOMENTE dentro de stComponentContainer.
   Os uploaders reais ficam dentro de stFileUploader — seletor diferente,
   não são afetados.
   ════════════════════════════════════════════════════════════════════════════ */
...
/* Mantém apenas o iframe visível dentro do container */
...
/* Uploaders legítimos (página 1) — garantia de visibilidade */
[data-testid="stFileUploader"] section {
  display: flex !important;
}
...
with st.sidebar:
    ...
    _PAGES = ["Carregar Arquivos", "Gerar Tabela Final", "Tabela Final"]
    ...
    # Marca o item ativo via JS — localiza o botão pelo seu texto e aplica estilo
...
# ─── Roteamento ───────────────────────────────────────────────────────────────
p = page  # page já é o nome sem prefixo numérico
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    _page_header("Tabela Final",
                 "Visualização · Dashboard · Exportação")
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  2. TABELA INTERATIVA — filtros inline estilo Excel nos cabeçalhos
    # ══════════════════════════════════════════════════════════════════════
    st.markdown(
        '<p style="font-size:11px;color:#6B7280;margin:0 0 6px;">'
        'Use o simbolo ao lado de cada coluna para filtrar, '
        'ou clique no nome da coluna para ver o dashboard.</p>',
        unsafe_allow_html=True,
    )
    # Div âncora com height fixa — ocupa o espaço antes do iframe montar,
    # impedindo que o placeholder de loading seja visível.
    st.markdown(
        f'<div style="height:490px;background:#F8F9FA;overflow:hidden;'
        f'position:relative;margin-bottom:-490px;z-index:0;"></div>',
        unsafe_allow_html=True,
    )
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  3. DASHBOARD DA COLUNA (abaixo da tabela)
    # ══════════════════════════════════════════════════════════════════════
    if sel_col:
        _divider()
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  4. EXPORTAÇÕES — APARECEM IMEDIATAMENTE
    #
    #  Padrão: os botões renderizam ANTES de qualquer geração de arquivo.
    #  CSV é gerado direto (leve). Excel e PPTX são gerados sob demanda
    #  com st.session_state — só processam quando o usuário clica em "Gerar".
    #  Após gerado, o botão vira "Baixar" (download_button) sem reprocessar.
    # ══════════════════════════════════════════════════════════════════════
    _divider()
    st.markdown(
    ...
    # Excel — gera sob demanda, fica em session_state
    with ex2:
