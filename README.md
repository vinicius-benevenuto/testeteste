...
def _resolve_diagram_image() -> str:
    """Resolve o caminho da imagem do diagrama em múltiplas localizações."""
    ...
    # 2. Caminhos relativos ao projeto (Linux / Docker)
    candidates = [
        _os.path.join(base, "static",    "diagram.png"),
        _os.path.join(base, "templates", "diagram.png"),
        _os.path.join(base, "static",    "20251007_140942_0000.png"),
        _os.path.join(base, "templates", "20251007_140942_0000.png"),
    ]
    for p in candidates:
        if _os.path.exists(p):
            return p

    # 3. Se env var definida mas arquivo não existe, retornar mesmo assim
    #    (builder vai logar warning e pular a imagem)
    if env:
        return env

    # 4. Fallback: static/diagram.png (será logado como não encontrado)
