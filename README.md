...
const SBC = (function(){
  ...
  function cpuBarClass(pct){
    if(pct<=40) return 'low'; if(pct<=60) return 'mid'; if(pct<=80) return 'high'; return 'crit';
  }
  ...
  function renderCards(data){
    ...
    data.sbcs.forEach((sbc)=>{
      ...
      const avgVal = sbc.cpu_avg != null ? sbc.cpu_avg : sbc.cpu_media;
      card.innerHTML = `
        ...
        <div class="sbc-cpu-bar-wrap">
          <div class="sbc-cpu-bar ${cpuBarClass(avgVal)}" style="width:${Math.min(100,avgVal)}%"></div>
        </div>
        <div class="sbc-card-details">
          <span>Modelo: <strong>${sbc.modelo||'—'}</strong></span>
          <span>CPU avg: <strong>${avgVal}%</strong> · máx: ${sbc.cpu_max}%</span>
