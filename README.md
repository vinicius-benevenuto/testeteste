...
class PTIWorkbookBuilder:
    ...
    def _b_conc(self):
        ...
        # ===== LINHA VAZIA EXTRA (para inclusão manual) =====
        extra_start = ds + num_data
        for i in range(3):
            r = extra_start + i
            ws.row_dimensions[r].height = 22.0
            f = s.alt_fill(num_data + i)
    ...
    def _b_prefixes(self):
        """
        Aba Plan Num_Oper — Tabela de Prefixos da Operadora.

        Auto-preenche a partir de:
          - self.op_rows (dados da operadora: cn, cidade, uf, eto_lc, cnl, cod_cni)
          - self.vivo_rows (cross-ref via CN)
          - CN_METADATA (município / UF fallback)
          - self.traffic (extração de CSP)
          - self.form (rn1, csp flag, cng flag)
        """
        import re as _re
        s = self.s
        ...
        # --- Larguras base ---
        col_w = {1: 2, 2: 5, 3: 16, 4: 14, 5: 12, 6: 7, 7: 12, 8: 12, 9: 10, 10: 16}
        max_w = dict(col_w)
        last_col = 10
        ...
        # ===== BANNER + TÍTULO =====
        self._bh(ws, 2, 2, last_col)
        self._st(ws, 4, 2, last_col,
                 f"2.6. Tabela de Prefixos da {n.upper()}")

        # ===== OBS =====
        ws.merge_cells(start_row=5, start_column=2,
                       end_row=5, end_column=last_col)
        c = ws.cell(row=5, column=2,
                    value='OBS: Caso necessite incluir mais faixas, inserir '
                          'linhas adicionais (coluna B - "Ref").')
        ...
        # =============================================================
        h1 = 7
        ws.row_dimensions[h1].height = 28.0
        hdr1 = [
            (2, 2, "Ref", True),
            (3, 3, "Município", True),
            (4, 4, "ÁREA LOCAL", True),
        ...
        for sc, ec, txt, vertical in hdr1:
            if vertical:
                ws.merge_cells(start_row=h1, start_column=sc,
                               end_row=h1 + 1, end_column=ec)
            else:
                ws.merge_cells(start_row=h1, start_column=sc,
                               end_row=h1, end_column=ec)
        ...
        # CABEÇALHO NÍVEL 2 (sub-colunas SERIE AUTORIZADA)
        h2 = h1 + 1
        ws.row_dimensions[h2].height = 32.0
        ...
        # =============================================================
        # DADOS — auto-preenchimento a partir de op_rows + cross-ref
        # =============================================================
        ds = h2 + 1
        data_rows = self.op_rows if self.op_rows else []
        num_rows = max(len(data_rows), 1)

        for i in range(num_rows):
            r = ds + i
            d = data_rows[i] if i < len(data_rows) else {}
            f = s.alt_fill(i)
            ws.row_dimensions[r].height = 22.0

            # --- Extrair campos ---
            cn_val = d.get("cn", "").strip()
            cidade = d.get("cidade", "").strip()
            uf_val = d.get("uf", "").strip()
            cnl_val = ""    # op_rows não tem cnl; conc pode ter
            eot_lc = d.get("eto_lc", "").strip()
            localidade = d.get("localidade", "").strip()

            # Cross-ref com conc para cnl / cod_cni
            for cd in self.conc:
                if cd.get("cn", "").strip() == cn_val and cn_val:
                    cnl_val = cnl_val or cd.get("cnl", "").strip()
                    break

            # Fallback CN_METADATA
            if cn_val and (not cidade or not uf_val):
                meta = CN_METADATA.get(cn_val.zfill(2))
                if meta:
                    if not cidade:
                        cidade = meta[0]
                    if not uf_val:
            ...
            # Área Local = Município (neste contexto)
            area_local = cidade

            # Ref automático
            ref_val = d.get("ref", "").strip() or str(i + 1)

            # --- Preencher células ---
            row_data = {
                2:  ref_val,
                3:  cidade or "[FALTA: Município]",
                4:  area_local or "[FALTA: Área Local]",
                5:  cnl_val,
                6:  cn_val or "[FALTA: CN]",
                7:  "",   # SERIE INCLUÍNOS (manual)
                8:  "",   # SERIE FINAL (manual)
                9:  eot_lc,
                10: "",   # SNOA (manual)
            }

            for col_num in range(2, last_col + 1):
                val = row_data.get(col_num, "")
                c = ws.cell(row=r, column=col_num, value=val)
                c.border = s.box_border
                c.fill = f
                if isinstance(val, str) and val.startswith("[FALTA:"):
        ...
        # =============================================================
        # SEÇÃO RESUMO — RN1 / EOT / CSP / CNG / RN2
        # =============================================================
        ps = ds + num_rows + 1

        # Extrair valores do formulário
        rn1_val = (row_get(form, "rn1", "") or "").strip()
        csp_flag = row_get(form, "csp", "0")
        cng_flag = row_get(form, "cng", "0")

        # Extrair CSP numérico dos tipos de tráfego
        csp_codes = set()
        for t in self.traffic:
            m = _re.search(r'CSP\s*(\d{2})', t, _re.IGNORECASE)
            if m:
                csp_codes.add(m.group(1))
        csp_str = ", ".join(sorted(csp_codes)) if csp_codes else (
            "Sim" if str(csp_flag) in ("1", "true", "True") else ""
        )

        # Extrair EOT Local / EOT LDN/LDI dos op_rows
        eots_local = set()
        for d in data_rows:
            v = d.get("eto_lc", "").strip()
            if v:
                eots_local.add(v)
        eot_local_str = ", ".join(sorted(eots_local))
        ...
        for t in self.traffic:
            if "CNG" in t.upper():
                cng_val = "Sim"
                break
        if not cng_val and str(cng_flag) in ("1", "true", "True"):
            cng_val = "Sim"

        summary_items = [
            ("RN1",              rn1_val),
            ("EOT Local",        eot_local_str),
            ("EOT LDN/LDI",     ""),
            ("CSP",              csp_str),
            ("Código Especial:", ""),
            ("CNG:",             cng_val),
        ...
        # Notas de transporte (mescladas)
        transport_notes = [
            "Operadora de transporte nas localidades onde não existir rota direta",
            "Transbordo nas localidades onde existe rota direta:",
        ]
        for offset, txt in enumerate(transport_notes):
            r = ps + offset
        ...
        # =============================================================
        # SEÇÃO UF / SERIE AUTORIZADA
        # =============================================================
        tr = ps + len(summary_items) + 1
        for sc, ec, txt in [(2, 3, "UF"), (4, 7, "SERIE AUTORIZADA")]:
            ws.merge_cells(start_row=tr, start_column=sc,
                           end_row=tr, end_column=ec)
            c = ws.cell(row=tr, column=sc, value=txt)
            c.font = s.font_subheader
            c.alignment = align_wrap
            c.fill = s.fill_secondary
            c.border = s.box_border
        ws.row_dimensions[tr].height = 25.0

        # Auto-preencher UFs únicos
        ufs_unicos = []
        seen_ufs = set()
        for d in data_rows:
            uf = d.get("uf", "").strip()
            if not uf:
                cn = d.get("cn", "").strip()
                meta = CN_METADATA.get(cn.zfill(2)) if cn else None
                uf = meta[1] if meta else ""
            if uf and uf not in seen_ufs:
                ufs_unicos.append(uf)
                seen_ufs.add(uf)

        uf_rows = max(len(ufs_unicos), 1)
        for i in range(uf_rows):
            r = tr + 1 + i
            ws.row_dimensions[r].height = 22.0
            f = s.alt_fill(i)
            uf_val = ufs_unicos[i] if i < len(ufs_unicos) else ""

            # UF (merged 2-3)
            ws.merge_cells(start_row=r, start_column=2,
                           end_row=r, end_column=3)
            c = ws.cell(row=r, column=2, value=uf_val)
            c.font = s.font_small
            c.alignment = align_wrap
            c.border = s.box_border
            c.fill = f

            # SERIE AUTORIZADA (merged 4-5, 6-7) — vazias
            for sc, ec in [(4, 5), (6, 7)]:
        ...
        cr = tr + 1 + uf_rows + 1
        cng_notes = [
            "A programação de CNG ocorre pelo processo de portabilidade "
            "intrínseca - via ABR",
            "O encaminhamento do CNG será programado nas localidades onde "
            "o ponto de entrega for informado",
        ]
        for offset, txt in enumerate(cng_notes):
            r = cr + offset
            ws.merge_cells(start_row=r, start_column=2,
                           end_row=r, end_column=last_col)
        ...
        # REFERÊNCIA VIVO SMP
        # =============================================================
        sm = cr + len(cng_notes) + 1
        ws.merge_cells(start_row=sm, start_column=2,
                       end_row=sm, end_column=last_col)
        c = ws.cell(row=sm, column=2, value="2.12 Tabela de Prefixos Vivo SMP")
        c.font = Font(name="Calibri", size=10, bold=True)
        ws.row_dimensions[sm].height = 22.0

        ws.merge_cells(start_row=sm + 1, start_column=2,
                       end_row=sm + 1, end_column=last_col)
        c = ws.cell(row=sm + 1, column=2,
                    value="Os prefixos da VIVO SMP, poderão ser obtidos "
                          "acessando o site www.telefonica.net.br/sp/transfer/")
        c.font = s.font_small
        c.alignment = align_left_wrap
        ws.row_dimensions[sm + 1].height = 22.0
