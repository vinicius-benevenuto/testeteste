...
class PTIWorkbookBuilder:
    ...
    def _extract_vivo_rows(self) -> list[dict]:
        keys = ("ref", "data", "escopo", "localidade", "cn", "bloco_ip", "mask",
    ...
    def _b_diagram(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            # Direita  (Operadora): faixa_ip_op + 2 (IP do IPAM reservado)
            ...
            ipam_base   = self._add_last_octet(ip_op_base, 2) if ip_op_base else ""
            ...
            for j in range(num_traffic):
                ...
                # Lado direito — IPAM + 2 (único valor, não incremental)
                ...
                op_ip_c = ws.cell(row=r, column=MID+1,
                                  value="" if num_visible > 1 else self._ip_only(ipam_base))
            ...
            if num_visible > 1:
                ...
                # IPAM: mesclar (único IP para todos os tráfegos)
                ws.merge_cells(start_row=data_start_row, start_column=MID+1,
                               end_row=data_start_row+num_visible-1, end_column=MID+1)
                mc = ws.cell(row=data_start_row, column=MID+1,
                             value=self._ip_only(ipam_base))
                ...
                ws.merge_cells(start_row=data_start_row, start_column=MID+2,
                               end_row=data_start_row+num_visible-1, end_column=MID+2)
                mc2 = ws.cell(row=data_start_row, column=MID+2, value=netmask_op)
                mc2.font = s.font_small; mc2.alignment = s.align_center; mc2.border = s.box_border
            ...
            ws.cell(row=r, column=MID,
                    value=f"IPAM ({n}): {ip_op_base or '___'}  →  +2: {self._ip_only(ipam_base) or '___'}").font = s.font_small
    ...
    def _b_routing(self) -> None:
        ...
        for b_idx in range(num_blocks):
            ...
            ponta_a_vivo   = pl_blk or bloco_ip_vivo or ""
            ...
            merged_cols = {
                ...
                21:(ip_op,           s.font_small),
                22:(mask_op,         s.font_small),
