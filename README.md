...
def run_mapping_wizard(
    ...
    # ── 1. Colunas para derivar UF ─────────────────────────────────────
    with st.expander("1️⃣ Colunas para derivar UF", expanded=True):
        st.caption("""
        O app usa **duas estratégias** para derivar UF — configure ambas:
        - **CN direto** (Science tem DDD como "43", "11"...) → via `CN → UF`
        - **CNL/PPI** (Portal tem código CNL como "41406") → via `CNL → CN → UF`
        """)

        st.markdown("**Estratégia A — CN direto no Science:**")
        cfg["cn_sci_col"] = _sel(
            "Coluna CN/DDD no Science",
            ["(nenhuma)"] + sci,
            _guess(sci, "Área Ponta B", "Area Ponta B", "CN", "DDD", "AREA_PONTA_B"),
            "wiz_cn_sci",
            "Coluna com o código DDD (ex: 43, 11, 47). O app mapeia direto para UF.",
        )

        st.markdown("**Estratégia B — CNL/PPI:**")
        cfg["cnl_sci_col"] = _sel(
            "Coluna CNL no Science", ["(nenhuma)"] + sci,
            _guess(sci, "CNL", "CNL_PPI", "Num SSI"),
            "wiz_cnl_sci",
            "Código CNL no Science (deixe em nenhuma se Science usa CN direto)",
        )
