...
<style>
  ...
  /* Campo vazio no modo Engenharia — destaque discreto */
  .ro input.empty-ro:not([type="hidden"]):not([type="checkbox"]),
  .ro select.empty-ro, .ro textarea.empty-ro {
...
/* ── Destaque de campos vazios no modo Engenharia ── */
if(ENG){
  const markEmpty = () => {
    document.querySelectorAll('.ro input:not([type="hidden"]):not([type="checkbox"]), .ro select, .ro textarea').forEach(el => {
      const empty = !(el.value || '').trim();
      el.classList.toggle('empty-ro', empty);
    });
  };
  // Marcar ao carregar
  markEmpty();
  // Re-marcar após o DOM ser populado pelas tabelas (aguarda addVivo/addOp)
  setTimeout(markEmpty, 800);
}
</script>
{% endblock %}
